%2 durative actions. Can start and end in different states.
...
%bump - durative exogenous action (processes in pddl+, we are not goint to use pddl+ just CASP) of a fixed duration
% realignment (-misaligned(robot)) no longer occurs when removing the battery.
% Instead the idea is the the robot is rocking back and forth and will stop after a period of time. 
% Durative exogenous action
start(vibrate(robot)) causes misaligned(robot)
end(vibrate(robot)) causes -misaligned(robot)
vibrate(robot) has duration Time
